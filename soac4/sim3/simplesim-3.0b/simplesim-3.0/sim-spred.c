 ...
 */
 ...
 static counter_t predictedValue=0;
 ...
 /* declaration of Register Prediction Table */
 LVPTaddrList RPT[64];
 ...
 /* which register? */
 static int reg; /* if it's 0, all registers are used */
 
 /*neural hibrid */
 static int k;
 
 /* training  0 - cu antrenare statica  1- fara antrenare statica*/
 static int train;
 
 /* training  type 0 - antrenare dinamica  1 -  antrenare statica*/
 ...
 /* nr of iterations in case static + training*/
 ...
 void sim_reg_options(struct opt_odb_t *odb)
 {
   ...
   opt_reg_uint(odb, "-pred", "what to do {0-not distinct locality| 1-prediction| 2-distinct locality| 3-register prediction}",
				&predict, /* default */ 0,
				/* print */ TRUE, /* format */ NULL);
   ...
   opt_reg_uint(odb, "-contextual", "predictor type {0-stride| 1-contextual| 2- histeresis  hybrid| 3- neural hybrid}",
   ...
   opt_reg_uint(odb, "-reg", "which register? If it's 0, all registers are used",
				&reg, /* default */ 0,
				/* print */ TRUE, /* format */ NULL);
   opt_reg_uint(odb, "-k", "Neural History Behaviour ",
				&k, /* default */ 3,
				/* print */ TRUE, /* format */ NULL);
   opt_reg_uint(odb, "-trainingType", "Training type",
				&trainingType, /* default */ 0,
				/* print */ TRUE, /* format */ NULL);
   opt_reg_uint(odb, "-train", "Training ",
				&train, /* default */ 0,
				/* print */ TRUE, /* format */ NULL);
   opt_reg_uint(odb, "-iterations", "Iterations ",
				&iterations, /* default */ 10000,
				/* print */ TRUE, /* format */ NULL);
   opt_reg_uint(odb, "-threshold", "Threshold ",
				&threshold_int, /* default */ 0,
				/* print */ TRUE, /* format */ NULL);
   opt_reg_uint(odb, "-automat", "Automat ",
				&automat, /* default */ 0,
				/* print */ TRUE, /* format */ NULL);
   opt_reg_uint(odb, "-biti1", "Biti1 ",
				&biti1, /* default */ 0,
				/* print */ TRUE, /* format */ NULL);
   opt_reg_uint(odb, "-neural", "Neural ",
				&neural, /* default */ 0,
				/* print */ TRUE, /* format */ NULL);
 ...
 void sim_reg_stats(struct stat_sdb_t *sdb)
 {
   ...
   stat_reg_counter(sdb, "loadValueLocality",
	...
	stat_reg_counter(sdb, "predictedValue",
		"total number of predicted values",
		&predictedValue, 0, NULL);					
 ...
 void sim_main(void)
 {
   ...
   /* Deleting the "endproc.txt" file if exists */
   unlink("endproc.txt");
 
   threshold = threshold_int / 10;
   initializare();
   if (trainingType == 0) // dinaminc
	 generateRandomWeights();
   else
   {
	 if (train == 1) // static cu antrenare
	   generateRandomWeights();
	 else
	   loadWeights();
   }
 ...
 #include "machine.def"
	 ...
	 if ((predict == 3) && (((reg == 99) && (out1 == 1 || out1 == 7 || out1 == 17 || (out1 > 19 && out1 < 24) || out1 == 29 || out1 == 30 || out1 == 31)) || (reg == out1))) /* register value prediction */
	 {
	   sim_num_loads++;
	   if (RPT[out1] == NULL)
	   {
		 ...
		 foundAssociativeLVPTAddress(out1, regs.regs_R[out1], history, &RPT[out1], 1, contextual, pattern, k, out1, trainingType, train, iterations, threshold, automat, biti1, neural);
	 }
	 ...
	 if (MD_OP_FLAGS(op) & F_LOAD)
	 {
	   if (predict == 0 || predict == 1 || predict == 2)
		 sim_num_loads++;
	   if (predict == 1) /* load value prediction */
	   {
		 if (isAssoc)
		 {
		   if (memAddress)
		   {
			 if (!foundAssociativeLVPTAddress(addr, regs.regs_R[out1], history, &lvpt, LVPTdim, contextual, pattern, k, out1, trainingType, train, iterations, threshold, automat, biti1, neural))
			 {
		   ...
		   {
			 if (!foundAssociativeLVPTAddress(regs.regs_PC, regs.regs_R[out1], history, &lvpt, LVPTdim, contextual, pattern, k, out1, trainingType, train, iterations, threshold, automat, biti1, neural))
			 {
		 ...
		 {
		   if (memAddress)
			 insertIntoDirrectMappedLVPT(addr, regs.regs_R[out1], history, &lvpt, contextual, LVPTdim, pattern, k, out1, trainingType, train, iterations, threshold, automat, biti1, neural);
		   else
			 insertIntoDirrectMappedLVPT(regs.regs_PC, regs.regs_R[out1], history, &lvpt, contextual, LVPTdim, pattern, k, out1, trainingType, train, iterations, threshold, automat, biti1, neural);
		 }
	   }
	   else if (predict == 0 || predict == 2) /* load value locality verification */
	   {
		 if (memAddress)
		 {
		   if (l == NULL)
			 l = pushAddress(l, addr, regs.regs_R[out1]);
		   else if (!foundAddress(l, addr, regs.regs_R[out1], history, predict))
			 l = pushAddress(l, addr, regs.regs_R[out1]);
		 }
		 else
		 {
		   ...
		   else if (!foundAddress(l, regs.regs_PC, regs.regs_R[out1], history, predict))
			 l = pushAddress(l, regs.regs_PC, regs.regs_R[out1]);
		 }
	   }
	 }
	 ...
	 /* Quit the program if the "endproc.txt" file exists */
	 if (sim_num_insn % 1000 == 0)
	 {
	   FILE *file;
	   file = fopen("endproc.txt", "r");
	   loadValueLocality = get_locality();
	 ...
	 {
	   loadValueLocality = get_locality();
	   predictedValue = get_predictValue();
