...
static unsigned int number_of_prime_numbers;
...
static int prime_number_vector[1000000];
...
void num_prime(int output[], int n){
	int contor=0;
	int currentNumber=0;
	while(contor<n){
		if(isPrim(currentNumber)){
			output[contor]=currentNumber;
			contor++;
		}
		currentNumber++;
...
"  Predictor `comb' combines a bimodal and a 2-level predictor.\n"
  ...
  opt_reg_uint(odb, "-contor:LD", "number of LOAD/STORE",
	       &opt_load, /* default */0,
	       /* print */TRUE, /* format */NULL);

  opt_reg_uint(odb, "-history", "number of prime numbers",
	       &number_of_prime_numbers, /* default */0,
	       /* print */TRUE, /* format */NULL);
...
sim_reg_stats(struct stat_sdb_t *sdb)
{
  ...
  if(opt_load==1){
  stat_reg_counter(sdb, "sim_num_of_loads",
		   "total number of loads executed",
		   &contor_loads, 0, NULL);
  }
  else{
  stat_reg_counter(sdb, "sim_num_of_stores",
...
sim_init(void)
{
  num_prime(prime_number_vector, number_of_prime_numbers);
  sim_num_refs = 0;
  contor_loads=0;
  contor_stores=0;
...
sim_main(void)
{
  ...
  for(int i=0;i<number_of_prime_numbers; i++){
	  printf("%d",prime_number_vector[i]);
	  printf("\n");
  }
